...
function LoginContent() {
  ...
  const handleSetUser = (user: any, jwtToken?: string) => {
    // sessionToken: Parse session token（用于 Parse API）
    // jwtToken: JWT（用于后端 API）
    const parseSessionToken = user.sessionToken;
    console.log('[Login] handleSetUser - JWT token:', jwtToken ? jwtToken.substring(0, 30) + '...' : 'undefined');
    console.log('[Login] handleSetUser - Parse sessionToken:', parseSessionToken ? parseSessionToken.substring(0, 30) + '...' : 'undefined');
    setUser({
      objectId: user.objectId,
      sessionToken: parseSessionToken,  // Parse session token
      jwtToken: jwtToken,               // JWT token for backend API
